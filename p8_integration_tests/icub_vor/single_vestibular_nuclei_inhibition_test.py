...
# ######################################################################################################
# This projection is here only to make sure the correct binary is loaded - i.e.  the one with plasticity
mossy_fibre_src = p.Population(1, # number of sources
                        p.SpikeSourceArray, # source type
                        {'spike_times': []}, # source spike times
...
pc_src = p.Population(1, # number of sources
                        p.SpikeSourceArray, # source type
...
p.Projection(pc_src, vestibular_neuclei, p.AllToAllConnector(), 
             p.StaticSynapse(weight=0.1, delay=1.0),
             receptor_type="inhibitory") 



pc_src.record('spikes')
vestibular_neuclei.record("all")

p.run(1000)

pc_src_spikes = pc_src.get_data('spikes')
vestibular_neuclei_data = vestibular_neuclei.get_data('all')
...
F = Figure(
    ...
    Panel(pc_src_spikes.segments[0].spiketrains,
    ...
    Panel(vestibular_neuclei_data.segments[0].filter(name='gsyn_inh')[0],
