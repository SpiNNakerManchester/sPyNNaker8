...
def do_run(plot):
    ...
    for group_index in range(n_groups-1):
        p.Projection(exc_pops[group_index % n_groups],
                     ...
                     p.FixedNumberPostConnector(90, rng=rng,
    ...
    p.Projection(pop_stim, inh_pops[0],
                 p.FixedNumberPostConnector(50, rng=rng),
...
class FixedNumberPostConnectorTest(BaseTestCase):
    def test_run(self):
        ...
        self.assertGreater(2600, len(spklist_exc))
        self.assertGreater(1700, len(spklist_inh))
        self.assertLess(1400, len(spklist_exc))
        self.assertLess(1200, len(spklist_inh))
