...
# NOTICE Multiple timesteps in one Vertex is NOT recommended
# This is purely a hacked together proof of concept
# This shows what would need to be done to support multiple timesteps
# It shows how the recorded data would have to be full of nan values
# This allows a test to be run
...
class MultiPopulationVertex(
    ...
    def __init__(
        ...
        super(MultiPopulationVertex, self).__init__(
    ...
    def _get_buffered_sdram(self, vertex_slice, data_simtime_in_us):
        ...
        for variable in self.__neuron_impl.get_recordable_variables():
            values.append(
                self._nr_by_slice(vertex_slice).get_buffered_sdram(
    ...
    def _write_neuron_parameters(
        ...
        recording_data = self._nr_by_slice(vertex_slice).get_data(vertex_slice)
    ...
    def is_recording_spikes(self):
        for neuron_recorder in self.__neuron_recorders:
            if neuron_recorder.is_recording("spikes"):
                return True
        return False
    ...
    def is_recording(self, variable):
        for neuron_recorder in self.__neuron_recorders:
            if neuron_recorder.is_recording(variable):
                return True
        return False
    ...
    def read_parameters_from_machine(
        ...
        offset = self._nr_by_slice(vertex_slice).get_sdram_usage_in_bytes(
            vertex_slice)
    ...
    def clear_spike_recording(self, buffer_manager, placements, graph_mapper):
        self._clear_recording_region(
            ...
            MultiPopulationVertex.SPIKE_RECORDING_REGION)
