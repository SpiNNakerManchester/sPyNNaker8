...
class SynfireProjectionOnSameChip(BaseTestCase):
    ...
    def get_before_and_after(self):
        ...
        self.assertEqualss(n_neurons, len(weights[0]))
        self.assertEqualss(n_neurons, len(weights[1]))
        self.assertTrue(numpy.allclose(weights[0][0][2], weights[1][0][2]))

        delays = synfire_run.get_delay()
        self.assertEqualss(n_neurons, len(delays[0]))
        self.assertEqualss(n_neurons, len(delays[1]))
