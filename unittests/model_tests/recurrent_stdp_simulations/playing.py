...
p.set_number_of_neurons_per_core(p.extra_models.IFCondCombExp2E2I, 4) # was 80)
p.set_number_of_neurons_per_core(p.extra_models.IFCurrCombExp2E2I, 4) # was 80)
p.set_number_of_neurons_per_core(p.SpikeSourceArray, 200)

nSourceNeurons = 32000 # number of input (excitatory) neurons
nExcitNeurons  = 32 # number of excitatory neurons in the recurrent memory
...
total_delay    = 1*timeStep   # 5*timeStep # ms # works with 5, 4
...
#-----
delay_e2e      = RandomDistribution('uniform', (timeStep, 16*timeStep), rng=rng) # was 0.8 # ms
pconn_e2e      = 0.1
...
populations[excit].record(['v', 'gsyn_exc'], indexes=[0,1,2,3,4,5])
...
print "There are ", len(spikes), " memory spikes!"
...
if True:
   ...
   while (gotAll != True and failCount < 10):
      try:
         excitGsyn = populations[excit].get_data('gsyn_exc')
         gotAll = True
      except:
         failCount += 1
         final_weights = list()
...
print memPots.segments[0].filter(name='v')
doPlots = True
if doPlots:
    Figure(
        Panel(stimSpikes.segments[0].spiketrains,
              yticks=True, ylabel="Stimulus ID", markersize=0.2, xticks = True, xlabel = "Stim Time (ms)", xlim=(0, runTime)),
        Panel(teachSpikes.segments[0].spiketrains,
              yticks=True, ylabel="Teach ID", markersize=0.2, xticks = True, xlabel = "Teach Time (ms)", xlim=(0, runTime)),
        Panel(spikes.segments[0].spiketrains,
              yticks=True, ylabel="Memory ID", markersize=0.2, xticks = True, xlabel = "Memory Time (ms)", xlim=(0, runTime)),
        ...
        Panel(excitGsyn.segments[0].filter(name='gsyn_exc')[0],
              ylabel="gsyn excitatory (mV)",
              data_labels=[populations[excit].label], yticks=True, xlim=(0, runTime))
