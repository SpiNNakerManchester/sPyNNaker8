...
pop_input = p.Population(n_neurons,
                         p.SpikeSourcePoisson(
                             rate=80, start=0, duration=runtime),
                         label="Poisson input broad feedforward cells")
...
proj_input = p.Projection(pop_input, pop_exc_gc,
                          p.OneToOneConnector(),
                          receptor_type="excitatory",
                          synapse_type=p.StaticSynapse(weight=1.0, delay=1.0),
                          label="Broad feedforward Poisson input cells excitatory connections to grid cells")
...
plt.savefig(data_dir + "sample_gsyn_exc.eps", format='eps')
plt.show()
