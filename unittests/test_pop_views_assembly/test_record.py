...
class TestPopulation(BaseTestCase):
    ...
    def test_depricated(self):
        ...
        assert [] == pop._get_all_recording_variables()
        pop.record_v()
        pop.record_gsyn()
    ...
    def test_set_many(self):
        ...
        assert target == set(pop._get_all_recording_variables())
        target1 = [2, 3, 4]
        assert target1 == pop._neuron_vertex._neuron_recorder._indexes["v"]
        view2 = pop[4:7]
        view2.record("v")
        target2 = [2, 3, 4, 5, 6]
        assert target2 == pop._neuron_vertex._neuron_recorder._indexes["v"]
        assert target1 == pop._neuron_vertex._neuron_recorder._indexes["spikes"]
        sim.end()
    ...
    def test_record_all_of_by_indexes(self):
        ...
        view1 = pop[0:3]
        view1.record(None)
        assert [3, 4] == pop._neuron_vertex._neuron_recorder._indexes["v"]
        assert {"v"} == set(pop._get_all_recording_variables())
        view2 = pop[3:]
        view2.record(None)
        assert pop._neuron_vertex._neuron_recorder._indexes["v"] is None
